...
**Phase 2 — Historical Decision Core: current; 2A implemented on `phase-2/projections` at `816b2db` (P2-A-02 corrections landed), awaiting ChatGPT re-review and user merge authorization.** Phase 1 (Skeleton and One Trace) is complete: accepted by ChatGPT review at `071da0a`, merge authorized by the user, merged into `main` as `ffa42bd` on 2026-09-04. Phase 0 completed at `2cdb8b5`; stack ratified (D-010) at `d183f60`.
...
- `main` = `7f86078` plus this STATE commit (content identical to `ffa42bd` except `STATE.md`), pushed, working tree clean.
- `phase-2/projections` = `816b2db` (four commits on `597bdff`; `816b2db` is the P2-A-02 correction: `available_at` joins evidence-version content identity, outcomes must occur strictly after their action, reason renamed `action_is_not_before_the_outcome`), pushed, not merged, no PR. Coordinator verification on that commit in a temporary worktree on the user's Mac (2026-09-06): `uv run pytest` 173 passed (137 acceptance/unit, 36 invariant); `ruff check` and `ruff format --check` clean; `reset && seed` → 9 created, second `seed` → 0 created, 9 duplicate; diff `1dd52f5..816b2db` touches only `collector/projections.py` and `tests/acceptance/test_cross_event_validation.py`. Earlier verification at `1dd52f5` (2026-09-04): projection counts `evidence_versions` 7, `logic_artifacts` 2, `decisions` 1 (86 / 75 / `PRIORITIZE` / `v3.2` / `db3a8bde…`), `decision_context` 8, `decision_consumed_inputs` 5, `persona_selections` 1, `actions` 1, `outcomes` 1; `/` lists only NovaSignal AI; `/accounts/_system` → 404; `/accounts/novasignal-ai` still seven rows with the synthetic banner. Branch CI at `816b2db`: run 34003913282, `lint` / `tests` / `invariants` green. `PRODUCT.md`, `DECISIONS.md`, `AGENTS.md`, `STATE.md`, templates, CSS, and `web/summaries.py` untouched on the branch.
...
The user sends the refreshed `.handoffs/review-packet-p2a.md` to ChatGPT for re-review; on ACCEPT and the user's authorization, the coordinator merges `phase-2/projections` at `816b2db` into `main` with `--no-ff` from the Mac shell and re-verifies on `main`; 2B is scoped only after that.
