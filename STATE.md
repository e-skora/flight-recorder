...
**Phase 2 — Historical Decision Core: current; 2A ACCEPTED by ChatGPT re-review at `816b2db` on 2026-09-05; awaiting the user's merge authorization.** Phase 1 (Skeleton and One Trace) is complete: accepted by ChatGPT review at `071da0a`, merge authorized by the user, merged into `main` as `ffa42bd` on 2026-09-04. Phase 0 completed at `2cdb8b5`; stack ratified (D-010) at `d183f60`.
...
On the user's explicit authorization, the coordinator merges `phase-2/projections` at `816b2db` into `main` with `--no-ff` from the Mac shell, re-verifies on `main` (suite, ruff, seed, CI), and reconciles this file; 2B (`evaluator-v1`, exact `v3.2` reconstruction, current-state isolation) is scoped only after that.
