...
class HistoricalContextEntry(StrictModel):
    ...
    def _reference_matches_availability(self) -> "HistoricalContextEntry":
        if self.availability == "available" and self.evidence_version_id is None:
            raise ValueError("an available input must reference an evidence_version_id")
        if self.availability == "unavailable" and self.evidence_version_id is not None:
            raise ValueError("an unavailable input must not reference any evidence")
        return self
