...
| --- | --- | --- | --- |
| 1 | Here is the operating company's account list, 241 synthetic accounts, with the canonical demo prospect pinned at the top. | `/` | `Canonical demo account: NovaSignal AI` |
| 2 | Open it and you get the whole account as a trace: discovery, two evidence records, the prioritization decision, the persona, the outbound action, the outcome, and the attribution of that outcome. | `/accounts/novasignal-ai` | `Account trace for NovaSignal AI (8 events)` |
| 3 | Open the decision. It scored 86 against a threshold of 75 and output `PRIORITIZE`, under workflow `v4.2` and logic `v3.2` — identified by artifact hash, not by the label. | `/accounts/novasignal-ai/decisions/evt-novasignal-04-decision-recorded` | `score 86 / threshold 75` |
| 4 | This is the context preserved at the boundary, with each input's provenance, availability time and state. Five inputs were consumed. Verified integration pressure was available and ignored. One input was explicitly unavailable. | `/accounts/novasignal-ai/decisions/evt-novasignal-04-decision-recorded#evidence-context` | `available but ignored` |
| 5 | Downstream: outbound play `#14` to the Head of Platform, synthetic cost `$1.42`, and 90 days later a recorded negative outcome — no reply, no meeting, no opportunity — attributed `direct` under a named policy. | `/accounts/novasignal-ai/decisions/evt-novasignal-04-decision-recorded#outcome-evt-novasignal-07-outcome-evaluated` | `opportunity: no (recorded negative observation)` |
| 6 | Across the whole dataset: the funding signal barely moves the observed rate, verified integration pressure moves it a lot, and workflow `v4.2` sits below its comparison cohort. Observed and descriptive — no cause is claimed. | `/insights` | `38.9% observed (58 of 149 eligible decisions; n = 149)` |
| 7 | Back on the decision, the current logic artifact `v5.1` is in effect, resolved by label to exactly one registered artifact and then used by hash. | `/accounts/novasignal-ai/decisions/evt-novasignal-04-decision-recorded#current-logic-selector` | `In effect: logic version v5.1` |
| 8 | The same preserved context under `v5.1`: 86 drops to 51, and the output flips to `DO_NOT_PRIORITIZE`. The ignored evidence is what moved it. No present-day evidence entered, and this result was not stored. | `/accounts/novasignal-ai/decisions/evt-novasignal-04-decision-recorded#replay-comparison` | `output changed: yes` |
